...
from app.core.generator import GENERATED_SOURCE_PROVIDER
...
async def _run(session: AsyncSession, job: orm.MigrationJob) -> None:
    if job.source_provider == GENERATED_SOURCE_PROVIDER:
        await _run_generated(session, job)
        return
...
async def _run_generated(session: AsyncSession, job: orm.MigrationJob) -> None:
    if job.status == "done":
        return
    target = get(job.target_provider)
    target_cred, _ = await load_fresh_credential(
    ...
    selection = job.selection or {}
    generated = selection.get("generated_playlist")
    if not isinstance(generated, dict):
        raise ValueError("generated migration is missing its playlist snapshot")
    playlist_name = generated.get("name")
    if not isinstance(playlist_name, str) or not playlist_name.strip():
        raise ValueError("generated migration has no playlist name")
    description = generated.get("description")
    if description is not None and not isinstance(description, str):
        raise ValueError("generated migration description is invalid")
    ...
    if not items:
        raise ValueError("generated migration has no confirmed tracks")
    if any(item.status in {"needs_review", "pending"} for item in items):
        raise ValueError("generated migration contains unreviewed tracks")
    ...
    target_playlist_id = next(
        (item.target_playlist_id for item in items if item.target_playlist_id),
        None,
    )
    if target_playlist_id and not await _target_playlist_exists(
        target,
        target_cred,
        target_playlist_id,
    ):
        target_playlist_id = None
    source_tracks = [_generated_item_track(item, job.target_provider) for item in items]
    if target_playlist_id is None:
        target_playlist_id = await _resolve_target_playlist(
            ...
            target=target,
            target_cred=target_cred,
            playlist_id=str(generated.get("draft_id") or job.source_account_id),
            playlist_name=playlist_name,
            description=description or "Generated by Open Playlist Engine.",
            playlist_kind=PlaylistKind.STANDARD,
    ...
    existing_keys = await _target_playlist_keys(target, target_cred, target_playlist_id)
    matched = [item for item in items if item.status == "matched"]
    chunk_size = _write_chunk_size(target)
    for offset in range(0, len(matched), chunk_size):
        await _write_matched_items(
            ...
            target_cred,
            target_playlist_id,
            matched[offset : offset + chunk_size],
    ...
    job.status = "done"
    await session.commit()
    logger.info("generated migration job_id=%s reached %s", job.id, Phase.DONE)


def _generated_item_track(item: orm.JobItem, target_provider: str) -> Track:
    return Track(
        id=str((item.source_metadata or {}).get("provider_track_id") or "") or None,
        title=item.title,
        artist=item.artist,
        album=item.album,
        ...
        isrc=item.isrc,
        provider_uris={target_provider: item.target_uri} if item.target_uri else {},
...
async def _resolve_target_playlist(
    ...
    *,
    ...
    previous = (
        await _previous_target_playlist_id(session, job=job, playlist_id=playlist_id)
        if reuse_previous
        else None
    )
