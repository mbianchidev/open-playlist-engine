...
async def _run(session: AsyncSession, job: orm.MigrationJob) -> None:
    ...
    if job.source_kind == "generated":
        await _run_generated(
            session,
            job,
            target=target,
...
async def _run_generated(
    ...
    *,
    ...
    if any(
        item.status in {"pending", "needs_review"} or not item.target_uri for item in items
    ):
...
def _generated_item_track(item: orm.JobItem, target_provider: str) -> Track:
    try:
        track = Track.model_validate(item.source_metadata or {})
